...
final class WMFAppViewController: UITabBarController, AppTabBarDelegate {
    ...
    func processUserActivity(_ activity: NSUserActivity, animated: Bool, completion done: @escaping () -> Void) -> Bool {
        ...
        case .places:
            dismissPresentedViewControllers()
            if let deepLink = activity.wmf_placesDeepLink {
                placesViewController.showExternalLocation(CLLocation(latitude: deepLink.latitude, longitude: deepLink.longitude))
            } else {
                placesViewController.clearExternalLocation()
            }
