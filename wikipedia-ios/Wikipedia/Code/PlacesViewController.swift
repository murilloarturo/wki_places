...
class PlacesViewController: ArticleLocationCollectionViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate {
    ...
    private var externalLocation: CLLocation?
    private var isUsingExternalLocation = false
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if isUsingExternalLocation {
            locationManager.stopMonitoringLocation()
            mapView.showsUserLocation = false
            applyExternalLocationIfNecessary()
            return
        }
    ...
    func showExternalLocation(_ location: CLLocation) {
        externalLocation = location
        isUsingExternalLocation = true
    ...
    func clearExternalLocation() {
        ...
        isUsingExternalLocation = false
        panMapToNextLocationUpdate = true
    }

    private func applyExternalLocationIfNecessary() {
        guard let externalLocation else {
            return
        }
        self.externalLocation = nil
        currentSearch = nil
        updateViewModeToMap()
        zoomAndPanMapView(toLocation: externalLocation)
    }
...
extension PlacesViewController {
    ...
    override func locationManager(_ locationManager: LocationManagerProtocol, didUpdateAuthorized authorized: Bool) {
        ...
        if authorized && !isUsingExternalLocation {
            panMapToNextLocationUpdate = currentSearch == nil
            locationManager.startMonitoringLocation()
        } else if isUsingExternalLocation {
            panMapToNextLocationUpdate = false
            locationManager.stopMonitoringLocation()
        } else {
            panMapToNextLocationUpdate = false
